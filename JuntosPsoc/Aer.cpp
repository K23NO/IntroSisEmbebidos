// main.c - Sistema unificado: Sensor de flujo YF-S401 + MAX30102 para PSoC 5LP
...
uint32_t currentFlowRate_x100 = 0;  // Flujo actual en formato x100
...
void processFlowSensor() {
    if (measurementReady) {
        ...
        currentFlowRate_x100 = (currentPulseCount * 85) / 75; // x100 para 2 decimales
    }
}

// ===== FUNCIÓN PARA ENVIAR RESULTADOS UNIFICADOS =====
void sendUnifiedResults() {
    // Formato: flujo, spo2, BPM
    sprintf(uartBuffer, "%lu.%02lu,%d,%d\r\n", 
            currentFlowRate_x100/100, currentFlowRate_x100%100, 
            (int)spO2, heartRate);
...
int main(void)
{
    ...
    // Mensajes iniciales
    sprintf(uartBuffer, "=== Sistema Unificado de Monitoreo ===\r\n");
    UART_1_PutString(uartBuffer);
    sprintf(uartBuffer, "Sensor de Flujo YF-S401 + MAX30102\r\n");
    UART_1_PutString(uartBuffer);
    ...
    sendString("Sistema iniciado. Formato: flujo(L/min), SpO2(%), BPM\r\n");
    
    // Variables para control de tiempo
    static uint32_t lastResultTime = 0;
    static uint32_t max30102Counter = 0;
