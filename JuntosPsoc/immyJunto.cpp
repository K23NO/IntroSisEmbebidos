...
int lastDiastolic = 0;
int lastSpO2 = 0;
uint32 lastFlowRate_x100 = 0;  // Almacena flujo con 2 decimales (valor x 100)
...
int main(void)
{
    ...
    // Inicialización de sensores de flujo
    UART_1_Start();
    Timer_Start();
    ...
    char initBuffer[128];
    sprintf(initBuffer, "Espirometro con sensor YF-S401 iniciado\r\n");
    UART_1_PutString(initBuffer);
    sprintf(initBuffer, "Configuracion: 115200 baud, 8N1, paridad par\r\n");
    UART_1_PutString(initBuffer);
    sprintf(initBuffer, "Iniciando mediciones... Estado pin: %d\r\n", FlowSensor_Pin_Read());
    UART_1_PutString(initBuffer);

    // Inicialización de sensores MAX30102
    I2C_1_Start();
    initSystemTick();
    spo2_init();
    CyDelay(2000);
    ...
    for(;;)
    {
        ...
        if(flowMeasurementReady)
        {
            ...
            // Debug: verificar conteo de pulsos
            sprintf(initBuffer, "Pulsos detectados: %lu\r\n", currentPulseCount);
            UART_1_PutString(initBuffer);
        }
