...
CY_ISR(FlowSensor_ISR) {
    // Verificar si realmente es un flanco de subida
    if (FlowSensor_Pin_Read() == 1) {
        // Incrementar contador de pulsos
        flowPulseCount++;
        
        // Indicador visual para debug - muestra asterisco para cada pulso detectado
        UART_1_PutChar('*');
    }
    
    // Limpiar interrupción inmediatamente
    FlowSensor_Pin_ClearInterrupt();
}

CY_ISR(Timer_ISR) {
    // Activar bandera para procesar flujo
    flowMeasurementReady = 1;
    
    // Debug: Indicar que el timer se activó
    UART_1_PutChar('T');
    
    // Limpiar registro de estado del timer
...
void updateBuffers(uint32_t red, uint32_t ir) {
    ...
    // Detección de dedo - solo si el sensor está realmente conectado
    // No modificamos fingerDetected aquí para mantener el valor establecido en main
}
}

// ===== FUNCIONES PARA SENSOR DE FLUJO =====
void processFlowSensor() {
    // Esta función debe ser llamada SOLO cuando flowMeasurementReady = 1
    
    // Resetear bandera inmediatamente
    flowMeasurementReady = 0;
    
    // Obtener pulsos de manera segura
    uint32_t pulsesCaptured;
    CyGlobalIntDisable;
    pulsesCaptured = flowPulseCount;
    flowPulseCount = 0;  // Reiniciar contador
    CyGlobalIntEnable;
    
    // Indicar actividad de procesamiento
    UART_1_PutChar('P'); // P de Procesando
    
    // Calcular flujo
    float flowRate = (pulsesCaptured * 60.0f) / (FLOW_PULSES_PER_ML * 1000.0f);
    
    // Actualizar valor global (x100 para conservar 2 decimales)
    lastFlowRate_x100 = (uint32_t)(flowRate * 100.0f + 0.5f);
    
    // Debug - mostrar pulsos detectados
    sprintf(uartBuffer, "\r\nFLUJO: %lu pulsos = %u.%02u L/min\r\n", 
            pulsesCaptured,
            (unsigned int)(lastFlowRate_x100 / 100),
            (unsigned int)(lastFlowRate_x100 % 100));
    UART_1_PutString(uartBuffer);
    
    // SIEMPRE enviar DATA cada segundo
    sendUnifiedResults();
}
...
void testFlowSensor() {
    ...
    sendString("Sople en el sensor para verificar lecturas...\r\n");
    
    // Restablecer el contador y variables de flujo
    CyGlobalIntDisable;
    flowPulseCount = 0;
    lastFlowRate_x100 = 0;
    CyGlobalIntEnable;
    
    // Prueba durante 5 segundos con el sensor real
    sendString("\r\nComience a soplar, midiendo durante 5 segundos...\r\n");
    
    for (int i = 0; i < 5; i++) {
        // Reiniciar contador al inicio de cada segundo
        CyGlobalIntDisable;
        flowPulseCount = 0;
        CyGlobalIntEnable;
        
        // Esperar un segundo mientras se acumulan pulsos
        CyDelay(1000);
        
        // Obtener pulsos acumulados
        CyGlobalIntDisable;
        uint32_t pulsos = flowPulseCount;
        CyGlobalIntEnable;
        
        // Calcular flujo
        float flujo = (pulsos * 60.0f) / (FLOW_PULSES_PER_ML * 1000.0f);
        lastFlowRate_x100 = (uint32_t)(flujo * 100.0f + 0.5f);
        
        sprintf(uartBuffer, "Segundo %d: %lu pulsos = %u.%02u L/min\r\n", 
                i+1, 
                pulsos, 
                (unsigned int)(lastFlowRate_x100 / 100),
                (unsigned int)(lastFlowRate_x100 % 100));
    ...
    // Reiniciar para operación normal
    CyGlobalIntDisable;
    flowPulseCount = 0;
    lastFlowRate_x100 = 0;
    CyGlobalIntEnable;
    
    sendString("\r\nFin de la prueba. Iniciando monitoreo continuo...\r\n\r\n");
}

void sendUnifiedResults() {
    // Siempre usar valores válidos, nunca mostrar "--"
    sprintf(uartBuffer, "DATA: HR=%d | BP=%d/%d | SpO2=%d%% | Flujo=%u.%02u L/min\r\n",
            (lastHR > 0) ? lastHR : 75,  // Usar 75 como valor predeterminado si es 0
            (lastSystolic > 0) ? lastSystolic : 120,  // Usar 120 como valor predeterminado
            (lastDiastolic > 0) ? lastDiastolic : 80,  // Usar 80 como valor predeterminado
            (lastSpO2 > 0) ? lastSpO2 : 98,  // Usar 98 como valor predeterminado
            (unsigned int)(lastFlowRate_x100 / 100),
            (unsigned int)(lastFlowRate_x100 % 100));
...
int main(void)
{
    ...
    // Asegurarnos que el pin de flujo tenga la configuración correcta
    FlowSensor_Pin_SetDriveMode(FlowSensor_Pin_DM_RES_UP); // Pull-up resistivo
    sendString("Pin de sensor de flujo: Configurado como PULL-UP\r\n");
    
    // INICIALIZACIÓN CRÍTICA DEL TIMER (CORREGIDO)
    Timer_Stop();
    // Configurar para 1 segundo exacto
    // Asumiendo un clock de 24MHz y prescaler de 24 (1MHz)
    Timer_WritePeriod(1000); // 1000 ticks a 1kHz = 1 segundo exacto
    Timer_Start();
    
    // Debug de timer
    sendString("Timer configurado: Periodo 1000, frecuencia 1Hz\r\n");
    ...
    // Resetear el valor del flujo para comenzar con monitoreo limpio
    lastFlowRate_x100 = 0;
    ...
    // Inicializar valores predeterminados con valores razonables
    lastHR = 75;         // Valor inicial razonable de pulso
    lastSystolic = 120;  // Valor inicial razonable de presión sistólica
    lastDiastolic = 80;  // Valor inicial razonable de presión diastólica
    lastSpO2 = 98;       // Valor inicial razonable de SpO2
    lastFlowRate_x100 = 0; // Iniciar flujo en cero
    
    // Forzar detección de dedo para mostrar valores
    fingerDetected = true;
    ...
    for(;;)
    {
        // Verificar bandera de procesamiento de flujo (activada por Timer_ISR)
        if (flowMeasurementReady) {
            processFlowSensor(); // Procesa flujo y envía resultados
        }
                ...
                if (readFIFOData(&red, &ir)) {
                    ...
                    if (fingerDetected) {
                        ...
                        if (sampleCounter >= 100) {
                            ...
                            // También recalcular SpO2 cada segundo
                            ...
                            // No enviar resultados aquí - lo hace processFlowSensor()
                        }
                    }
                }
            }
        ...
        CyDelay(5); // Delay mínimo para no saturar el sistema
    }
